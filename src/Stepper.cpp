...
#define HOMING_TAP_TOLERANCE          200
...
static int clampHomingSensitivity(int sensitivity, int threshold) {
  int maxSensitivity = min(HOMING_MAX_SENSITIVITY, max(threshold, 1));
  return constrain(sensitivity * 2, 1, maxSensitivity);
}
...
static HomingSgBaseline getHomingSgBaseline() {
  ...
  if (trimmedCount <= 0) {
    int threshold           = round(totalSgResult / (float)HOMING_SG_SAMPLE_COUNT);
    int measuredSensitivity = clampHomingSensitivity(userConfig->getHomingSensitivity(), threshold);
  ...
  measuredSensitivity     = clampHomingSensitivity(measuredSensitivity, threshold);
