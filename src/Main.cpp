...
void setup(){
  ...
  //Setup shift buttons for internal pullup resistors
  ...
  Serial.println("Configuring Stepper Pins");
...
{
  if(config.getWifiOn()){ //wifi is currently on, turn it off, turn BT client on.
...
bool IRAM_ATTR deBounce() {
  ...
  if ((millis() - lastDebounceTime) > debounceDelay) { //<----------------This should be assigned it's own task and just switch a global bool 
