...
double linearExtrapolate(const std::vector<double>& x, const std::vector<double>& y, double j) {
  double x0, x1, y0, y1;
    ...
    if (j < x.front()) {
        ...
        x0 = x[x.size() - 2], x1 = x[x.size() - 1];
        y0 = y[y.size() - 2], y1 = y[y.size() - 1];
    } else {
        auto upper = std::upper_bound(x.begin(), x.end(), j);
        auto lower = upper - 1;
        x0 = *lower, x1 = *upper;
        y0 = y[lower - x.begin()], y1 = y[upper - x.begin()];
    }
...
void PowerTable::findTableDirection(bool horizontal) {
  ...
  for (int outerValue = 0; outerValue < outerSize; ++outerValue) {
      ...
      for (const auto& [xi, yi] : unique_xy) {
          x.push_back(xi);
          y.push_back(yi);
      }
...
void PowerTable::fillEmptyTable(int outerValue, const std::vector<int>& emptyIndices, const std::vector<double>& x, const std::vector<double>& y, bool horizontal) {
  ...
  if (x.size() == 1) {  // If only one point, fill row with the value
      ...
      for (int innerValue : emptyIndices) {
        ...
        if (innerValue < x.front() || innerValue > x.back()) continue; 
...
void PowerTable::extrapFillTableDirection(bool horizontal) {
  ...
  for (int outerIndex = 0; outerIndex < outerSize; ++outerIndex) {
      ...
      extrapolateEmptyIndices(outerIndex, emptyIndices, x, y, horizontal);
  }
}

void PowerTable::extrapolateEmptyIndices(int outerIndex, const std::vector<int>& emptyIndices, const std::vector<double>& x, const std::vector<double>& y, bool horizontal) {
      int innerSize = horizontal ? POWERTABLE_WATT_SIZE : POWERTABLE_CAD_SIZE;
        ...
        for (int innerIndex : emptyIndices) {
          ...
          double extrapolated_value = s(innerIndex);
...
void PowerTable::extrapolateDiagonal() {
  std::vector<std::pair<double, double>> unique_xy; // Use vector for memory efficiency
  std::vector<std::pair<int, int>> emptyIndices;

  // Iterate over different diagonals
  for (int sum = 0; sum < POWERTABLE_CAD_SIZE + POWERTABLE_WATT_SIZE - 1; ++sum) {
      unique_xy.clear();
      emptyIndices.clear();

      // Collect known values for this diagonal (sum of indices is constant)
      for (int i = 0; i < POWERTABLE_CAD_SIZE; ++i) {
          int j = sum - i;
      ...
      std::sort(unique_xy.begin(), unique_xy.end()); // Sort by the 'x' coordinate (i)
