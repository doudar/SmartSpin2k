...
void TestLinearExtrapolate::test_linear_extrapolate(void) {
    ...
    for (int testCadence = 50; testCadence >= 30; testCadence -= 10) {
      ...
      if (positionValues.size() >= 2) {
        ...
        TEST_ASSERT_FLOAT_WITHIN_MESSAGE(
          ...
          expectedTrend,
          extrapolated,
          "Low cadence extrapolation should follow the trend of the first two data points");
      }
    }
  }
  ...
  for (int wattIndex = 0; wattIndex < POWERTABLE_WATT_SIZE; wattIndex++) {
    ...
    for (int testCadence = 110; testCadence <= 130; testCadence += 10) {
      ...
      if (positionValues.size() >= 2) {
        int lastIdx = cadValues.size() - 1;
        int secondLastIdx = cadValues.size() - 2;
        
        float expectedTrend = positionValues[lastIdx] + 
        ...
        TEST_ASSERT_FLOAT_WITHIN_MESSAGE(
          ...
          expectedTrend,
          extrapolated,
          "High cadence extrapolation should follow the trend of the last two data points");
      }
    }
  }
