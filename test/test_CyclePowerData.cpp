...
void test_cyclePowerData::test_parses_cadence(void) {
  ...
  // we would expect the repeated sample to maintain the previous cadence value
  sensor.decode(t7, sizeof(t7));
  TEST_ASSERT_EQUAL_FLOAT(56.39284, sensor.getCadence());

  // Test overflow
  sensor.decode(t12, sizeof(t12));
  sensor.decode(t13, sizeof(t13));
  TEST_ASSERT_EQUAL_FLOAT(31.09312, sensor.getCadence());
}
