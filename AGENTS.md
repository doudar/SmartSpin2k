...
- Thermal/UART safety runs every 10 seconds in maintenance, including during updates and homing. EN is held high until full TMC setup/current writes are acknowledged by IFCNT; failed communication and driver resets trigger reconfiguration. FastAccelStepper enable writes use a guarded external-pin callback so auto-enable cannot bypass a stop. UART operations are serialized across maintenance and homing; do not hold the driver mutex while waiting for motor movement.
- `include/ThermalSafety.h` owns the tested thermal policies. TMC T120 halves requested current and disables EN after 30 seconds without cooling; OT disables immediately, and only valid clear temperature flags release the latch. S3 radios reduce at 70 C (WiFi 8.5 dBm/modem sleep, BLE minimum -24 dBm including active links); motor current tapers from 100% at 70 C to 50% at 80 C, with EN high above 80 C until <=78 C. Radios restore below 68 C. Temperature sensor failure inhibits the motor. S3 logs `T=%dC` every 10 seconds using a dedicated IDF sensor configuration. Limits never change saved user current and apply to homing/setting writes too; the stricter of the two current limits wins.
